...
class SeparationPipeline:
    ...
    def process_batch(self, batch_dir: Path, mode: str = "learning", interactive: bool = False):
        ...
        console.print(f"[cyan]Processing {len(audio_files)} files...[/cyan]")
        ...
        batch_file = self.config["output_dir"] / f"batch_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def _separate_sources(self, audio_data: Dict, results: Dict, interactive: bool) -> Dict:
        ...
        if self.visualizer:
            self._create_stem_visualizations(separated["stems"], audio_data)
        ...
        if self.assistant:
            ...
            results["insights"].append({"stage": "separation", "content": insight})

        return separated

    def _save_stems(self, stems: Dict, audio_data: Dict) -> Dict:
        ...
        with Progress(SpinnerColumn(), TextColumn("Saving {task.description}...")) as progress:
            for stem_name, stem_audio in stems.items():
                task = progress.add_task(stem_name, total=1)
    ...
    def _create_stem_visualizations(self, stems: Dict, audio_data: Dict):
        """Create visualizations for each stem."""
        viz_dir = self.config["output_dir"] / "visualizations"
        filename = audio_data["filename"]

        for stem_name, stem_audio in stems.items():
            stem_viz_path = viz_dir / f"{filename}_{stem_name}.png"
            self.visualizer.create_spectrogram(
                stem_audio, audio_data["sample_rate"], stem_viz_path,
                f"{filename} - {stem_name.title()}"
            )

    def _analyze_stems(self, separated: Dict, results: Dict):
        ...
        for stem_name, stem_audio in separated["stems"].items():
            ...
            self._display_stem_analysis(stem_name, analysis)

        results["stages"]["stem_analysis"] = stem_analysis
    ...
    def _display_audio_analysis(self, analysis: Dict):
        ...
        # Recommendation
        if hasattr(self.analyzer, 'get_separation_recommendation'):
            rec = self.analyzer.get_separation_recommendation(analysis)
            console.print(f"\n[bold cyan]Recommendation:[/bold cyan] {rec}")

    def _display_stem_analysis(self, stem_name: str, analysis: Dict):
        """Display individual stem analysis."""
        table = Table(title=f"{stem_name.title()} Stem", show_header=True)
        table.add_column("Metric", style="cyan")
        table.add_column("Value", style="green")
    ...
    def _display_separation_metrics(self, metrics: Dict):
        ...
        table = Table(title="Separation Quality", show_header=True)
        table.add_column("Metric", style="cyan")
        table.add_column("Value", style="green")
    ...
    def _get_quality_grade(self, snr: float) -> str:
        if snr >= 25:
            return "A+"
        elif snr >= 20:
            return "A"
